...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.tachibana.downloader"
...
%.testlog: app-androidTest.apk app-instrumented.apk running-emulator
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.tachibana.downloader))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.tachibana.downloader; \
	else \
	...
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.tachibana.downloader ./traces/$(TEST_NAME) force-clean
...
copy-shadows :
	cp -v ./shadows/*.java app/src/carvedTest/com/tachibana/downloader
